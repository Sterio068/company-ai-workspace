...
- 重包安裝檔：`installer/dist/Company-AI-Installer.dmg`
...
- `.env`：`COMPANY_AI_DEFAULT_AI_PROVIDER=openai`
...
- 再次執行 `hdiutil verify installer/dist/Company-AI-Installer.dmg`：VALID
...
- 修補知識庫新增資料源提示：白名單文案改為 `/Volumes`、`/data`、`/tmp/company-ai-test-sources`，與後端限制一致。
- Playwright 手動資料流：登入、AI 引擎切換、工作包建立、交棒卡保存、CRM 商機詳情、流程草稿、媒體名單、社群排程、會計新交易引導、會議速記入口、場勘入口、Help、中文化掃描皆通過。
- Targeted retest：知識庫資料源以 `/tmp/company-ai-test-sources/qa-final-20260424151036` 建立並可在知識庫瀏覽；同仁管理建立 QA 帳號成功；console 0 非預期錯誤。
...
- `./scripts/smoke-test.sh`：8 通過 / 0 失敗
- `python3 -m pytest backend/accounting`：245 passed / 13 skipped
- `hdiutil verify installer/dist/Company-AI-Installer.dmg`：VALID
...
- 重新打包安裝檔：`installer/dist/Company-AI-Installer.dmg`，`hdiutil verify` VALID，source tar 已抽驗含 `app.js?v=72` 與 `_conversationExists`。
...
- Runtime 安全模式確認：`company-ai-accounting` 目前為 `NODE_ENV=production`、`ECC_ENV=production`、`ALLOW_LEGACY_AUTH_HEADERS=0`。
...
- 修補 Uptime 暴露面：`company-ai-uptime` 從 `0.0.0.0:3001` 改為 `127.0.0.1:3001`，只允許本機查看。
...
- DMG 重包：`installer/dist/Company-AI-Installer.dmg`，`hdiutil verify` VALID，SHA-256 `317a7b950830d566a0700c9eeb452a75526662712230c36ae5be37c5fc4be320`。
...
- 備份流程已做 `mongodump` stream + `gzip -t` 驗證，但暫存檔 `/tmp/company-ai-mongo-dump-stream.gz` 仍保留且已設 `600` 權限；未刪除是為避免擅自刪除本機資料。
